...
function requestHandler(req,proxyRsp) { 
	...
	function routeReq(callback){   
		var httpServerParams = _.find(config.httpServers, { 'url': path}); 
		console.log(path);
		if(!httpServerParams){    
			next = false;
			...
			requestInfo.route = _.find(config.routes, {'no' : httpServerParams.routeNo});
			requestInfo.response = _.find(config.response, {'no' : httpServerParams.responseNo});
	...
	function getSocketResonse(callback) {
		...
		client.on('data', function (data) {
			...
			// 解析内部服务返回报文
			var resParams = _.find(config.response, {'no' : requestInfo.route.responseNo});
